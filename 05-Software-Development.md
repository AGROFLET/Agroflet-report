...
## Anexo A — Guía de capturas del Capítulo V

Esta guía indica, para cada imagen referenciada en el capítulo, qué debe capturarse y con qué nombre debe guardarse dentro de `assets/images/`.

### A.1. Capturas de código (evidencia de desarrollo y convenciones)

| Nombre del archivo | Qué capturar exactamente |
| :--- | :--- |
| `Cap5_Convenciones_HTML.png` | `index.html`, **líneas 1 a 68**: `<!DOCTYPE html>`, `<head>` con metadatos, y el bloque `<header>` / `<nav>` mostrando clases BEM, `data-i18n`, `aria-label` y `aria-expanded`. |
| `Cap5_Convenciones_CSS.png` | `styles.css`, **líneas 6 a 83**: bloque `:root` completo con los design tokens (colores de marca, tipografía Inter, espaciados, radios, sombras y `--header-height`). |
| `Cap5_Convenciones_JS.png` | `app.js`, **líneas 1 a 30**: comentario de cabecera con el listado de features, `document.addEventListener('DOMContentLoaded', ...)`, `'use strict';` y el inicio del bloque `// ─── 1. Internationalization (ES/EN) ───`. |
| `Cap5_Dev_HTML.png` | `index.html`, **líneas 214 a 240**: apertura de `<section id="testimonials">` con `section__header`, `section__tag` y las tarjetas del carrusel. Alternativa igualmente válida: líneas 132 a 175 (sección de planes). |
| `Cap5_Dev_CSS.png` | `styles.css`, **líneas 872 a 910**: bloque `TESTIMONIALS` o bien **líneas 736 a 770** (`PLANS`), mostrando la nomenclatura BEM y una `@media (min-width: 768px)` visible en la misma captura. |
| `Cap5_Dev_JS.png` | `app.js`, **líneas 626 a 700**: bloque `// ─── 9. Testimonials Carousel with Touch & Swipe ───`. Alternativa: líneas 904 a 960 (validaciones `validateEmail`, `validatePhone`, `getPasswordStrength`). |
| `Cap5_Dev_i18n.png` | Vista dividida en VS Code con `assets/i18n/es.json` y `assets/i18n/en.json` abiertos lado a lado, mostrando las mismas claves (`hero.*`, `plans.*`) en ambos idiomas. |
| `Cap5_Repo_Estructura.png` | Panel **Explorer** de VS Code con el árbol del proyecto AGROFLET completamente expandido (`assets/css`, `assets/i18n`, `assets/images/team`, `assets/js`, `docs`, y los archivos de raíz). Esta captura ya la tienes tomada. |

### A.2. Capturas de la Landing Page en ejecución

Tómalas con el navegador en pantalla completa (1920 × 1080 recomendado) y sin la barra de marcadores visible.

| Nombre del archivo | Qué capturar exactamente |
| :--- | :--- |
| `Cap5_Exec_Hero_Desktop.png` | Sección `#hero` completa: header, badge, titular, subtítulo, botones "Empieza ahora" / "Conoce más" y los tres contadores estadísticos. |
| `Cap5_Exec_Planes_Desktop.png` | Sección `#plans`: las tres tarjetas (Básico, Estándar con cinta "Más Popular", Profesional) con precios y lista de funcionalidades. |
| `Cap5_Exec_Testimonios_Desktop.png` | Sección `#testimonials` con una tarjeta de testimonio visible, los botones de navegación y los indicadores (dots). |
| `Cap5_Exec_LaApp_Desktop.png` | Sección `#what-we-solve` con la grilla de seis tarjetas de funcionalidades y sus íconos. |
| `Cap5_Exec_Equipo_Desktop.png` | Sección `#team` con las cinco tarjetas de perfil del equipo fundador. |
| `Cap5_Exec_Videos_Desktop.png` | Sección `#videos` con las dos tarjetas de video institucional. |
| `Cap5_Exec_Footer_Desktop.png` | Footer completo: logo, descripción, columnas Enlaces / Legal / Contacto, copyright y redes sociales. |
| `Cap5_Exec_Terminos.png` | Página `docs/Terms-and-conditions.html` abierta en el navegador, mostrando el título y las primeras cláusulas. |
| `Cap5_Exec_Idioma_ES.png` | Hero en español con el selector del header mostrando **ES**. |
| `Cap5_Exec_Idioma_EN.png` | La **misma** sección Hero tras pulsar el selector, ahora en inglés y con el selector mostrando **EN** (es la prueba del criterio de aceptación de UH30). |
| `Cap5_Exec_Modal_Registro.png` | Modal de autenticación abierto en modo "Crear cuenta", con todos los campos visibles y el medidor de fortaleza de contraseña. |
| `Cap5_Exec_Validaciones.png` | El mismo modal tras intentar enviar con datos inválidos, mostrando los mensajes de error en rojo bajo los campos. |
| `Cap5_Exec_Mobile_Hero.png` | Hero en DevTools con dispositivo emulado **iPhone 14 Pro** o similar (390 × 844), en orientación vertical. |
| `Cap5_Exec_Mobile_Menu.png` | El mismo dispositivo emulado con el menú hamburguesa **abierto**, mostrando los enlaces de navegación y los botones de sesión. |
| `Cap5_Exec_DevTools_Responsive.png` | Pantalla completa de Chrome DevTools con el modo responsive activo y el selector de dispositivo visible junto a la página renderizada. |

### A.3. Capturas de despliegue y colaboración en GitHub

| Nombre del archivo | Qué capturar exactamente |
| :--- | :--- |
| `Cap5_Deploy_GitHubPages_Config.png` y `Cap5_Deploy_Settings_Pages.png` | Pantalla `Settings > Pages` del repositorio mostrando *Deploy from a branch*, la rama `main`, la carpeta `/ (root)` y el mensaje con la URL publicada. (Puede usarse la misma captura con ambos nombres o duplicarse el archivo.) |
| `Cap5_Deploy_Workflow.png` y `Cap5_Deploy_Actions.png` | Pestaña **Actions** con el workflow *pages build and deployment* finalizado con check verde. |
| `Cap5_Deploy_Produccion.png` | Navegador mostrando la Landing Page cargada desde la URL pública de GitHub Pages, **con la barra de direcciones visible** (es la evidencia de que está en producción). |
| `Cap5_Dev_Commits.png` | Pestaña **Commits** del repositorio mostrando el historial con los mensajes en formato Conventional Commits. |
| `Cap5_Dev_PullRequests.png` | Pestaña **Pull requests** (filtro *Closed*) mostrando los PR del Sprint 1 fusionados. |
| `Cap5_Collab_Branches.png` | Vista `Branches` del repositorio con las ramas `main`, `develop`, `release/v1.0.0` y las `feature/*`. |
| `Cap5_Collab_Commits_<Nombre>.png` | Pestaña **Commits** filtrada por autor (`?author=usuario`) para cada integrante, mostrando sus commits. |
| `Cap5_Collab_Contributors.png` | `Insights > Contributors`, con la gráfica de aportes de los cinco integrantes. |
| `Cap5_Collab_Histograma.png` | `Insights > Commits`, con el histograma semanal de contribuciones. |

### A.4. Logotipos de herramientas (sección 5.1.1)

Descargar el logotipo oficial de cada herramienta (formato PNG con fondo blanco o transparente, aproximadamente 500 px de ancho) y guardarlo con el nombre indicado en `assets/images/`:

`Cap5_Logo_Trello.png`, `Cap5_Logo_GoogleMeet.png`, `Cap5_Logo_Whatsapp.png`, `Cap5_Logo_UXPressia.png`, `Cap5_Logo_Miro.png`, `Cap5_Logo_Figma.png`, `Cap5_Logo_Lucidchart.png`, `Cap5_Logo_Structurizr.png`, `Cap5_Logo_VSCode.png`, `Cap5_Logo_HTML_CSS_JS.png`, `Cap5_Logo_LiveServer.png`, `Cap5_Logo_VueJS.png`, `Cap5_Logo_PrimeVue.png`, `Cap5_Logo_AspNetCore.png`, `Cap5_Logo_CSharp.png`, `Cap5_Logo_EFCore.png`, `Cap5_Logo_MySQL.png`, `Cap5_Logo_NodeJS.png`, `Cap5_Logo_Cucumber.png`, `Cap5_Logo_Git.png`, `Cap5_Logo_GitHub.png`, `Cap5_Logo_GitHubPages.png`, `Cap5_Logo_CloudHosting.png`, `Cap5_Logo_Markdown.png`, `Cap5_Logo_Swagger.png`.

### A.5. Datos pendientes de completar

| Marcador en el documento | Dato requerido |
| :--- | :--- |
| `[COMPLETAR: URL DEL REPOSITORIO]` | URL del repositorio de la Landing Page en GitHub. |
| `[COMPLETAR: URL DE GITHUB PAGES]` | URL pública de producción generada por GitHub Pages. |
| `[COMMIT_ID]` | Identificador corto (7 caracteres) de cada commit, obtenido con `git log --oneline`. |
| `[DD/MM/2026]` | Fecha real de cada commit. |
| Usuarios de GitHub de la tabla 5.2.1.2 | Reemplazar por las cuentas reales de cada integrante, verificables en `Insights > Contributors`. |
| Fecha y hora del Sprint Planning | Reemplazar por la fecha y hora reales de la reunión. |
| Video del Sprint Review | Enlace al video de ejecución alojado en SharePoint o Drive. |
